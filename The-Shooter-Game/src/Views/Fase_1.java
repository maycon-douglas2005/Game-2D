	...
	public class Fase_1 extends JPanel implements ActionListener {
			...
			addMouseListener(new java.awt.event.MouseAdapter(){
				...
				public void mouseClicked(MouseEvent e) {
					...
					if(!emJogo) {
						...
						if(cliqueX >= 410 && cliqueX <= 460 && cliqueY >= 390 && cliqueY <= 410) {
							reiniciarFase();
						}
						if(cliqueX >= 530 && cliqueX <= 550 && cliqueY >= 390 && cliqueY <= 410) {
							System.exit(0);
						}
					}
				}
		...
		public void paint(Graphics g) {
			...
			if(emJogo) {
				...
				graficos.drawString("Reiniciar", 410, 400);
				graficos.drawString("Sair", 530, 400);
